...
const FridgeModal: React.FC<FridgeModalProps> = ({ isOpen, onClose, toggleFridge }) => {
	// 初始化冷藏温度，从 localStorage 中读取，若无则设为默认值 4
	const [coldTemp, setColdTemp] = useState(() => {
		const storedTemp = localStorage.getItem('coldTemp');
		return storedTemp ? parseInt(storedTemp) : 4;
	});
	// 初始化冷冻温度，从 localStorage 中读取，若无则设为默认值 -18
	const [freezeTemp, setFreezeTemp] = useState(() => {
		const storedTemp = localStorage.getItem('freezeTemp');
		return storedTemp ? parseInt(storedTemp) : -18;
	});

	useEffect(() => {
		// 在组件卸载时保存当前的温度值到 localStorage
		return () => {
			localStorage.setItem('coldTemp', coldTemp.toString());
			localStorage.setItem('freezeTemp', freezeTemp.toString());
		};
	}, [coldTemp, freezeTemp]);
